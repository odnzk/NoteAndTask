package com.noteapp.feature_todolist.dialog.filters
...
import com.noteapp.feature_todolist.list.ListTodoEvent
import com.noteapp.feature_todolist.list.ListTodoViewModel
...
@AndroidEntryPoint
internal class FiltersTodoBottomSheetDialog : BottomSheetDialogFragment() {
    ...
    private fun displayFilters(filters: UiTodoFilters) = with(filters) {
        categories.toChipGroup(binding.chipgroupCategories) { selectedCategoryId ->
            viewModel.onEvent(ListTodoEvent.UpdateSelectedCategoriesId(selectedCategoryId))
        }
        with(binding) {
            when (period) {
                TodoFilterPeriod.NO_PERIOD -> {}
            ...
            additionalConditions.forEach { condition ->
                when (condition) {
                    TodoFilterAdditionalConditions.HIDE_COMPLETED ->
                        cbHideCompleted.isChecked = true
                    TodoFilterAdditionalConditions.HIDE_WITHOUT_DEADLINE ->
                        cbHideWithoutDeadline.isChecked = true
                }
            }
        }
